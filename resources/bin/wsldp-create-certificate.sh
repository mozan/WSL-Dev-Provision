...
PATH_CERTIFICATE=$WSLDP_GLOBAL_CONFIG_DIR/certificates
mkdir $PATH_CERTIFICATE 2>/dev/null

# Path to the custom WSL $(hostname) Root CA certificate.
PATH_ROOT_CNF="${PATH_CERTIFICATE}/ca.WSL.$(hostname).cnf"
PATH_ROOT_CRT="${PATH_CERTIFICATE}/ca.WSL.$(hostname).crt"
PATH_ROOT_KEY="${PATH_CERTIFICATE}/ca.WSL.$(hostname).key"

# Path to the custom site certificate.
PATH_CNF="${PATH_CERTIFICATE}/${1}.cnf"
PATH_CRT="${PATH_CERTIFICATE}/${1}.crt"
PATH_CSR="${PATH_CERTIFICATE}/${1}.csr"
PATH_KEY="${PATH_CERTIFICATE}/${1}.key"

BASE_CNF="
    [ ca ]
    default_ca = ca_WSL_$(hostname)

    [ ca_WSL_$(hostname) ]
    dir           = $PATH_CERTIFICATE
    certs         = $PATH_CERTIFICATE
    new_certs_dir = $PATH_CERTIFICATE
