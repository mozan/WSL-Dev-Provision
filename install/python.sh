...
if [ "$INSTALL_PYTHON" == "y" ]; then
	...
	if [ "$?" == "0" ]; then
		apt-get $APT_SILENCE install -y python3-pip build-essential libssl-dev libffi-dev python3-dev python3-venv python-django
		...
		sudo -H -u $INSTALLED_FOR_USER bash -c 'pip3 install django'
